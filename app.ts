...
function newsFeed(): void {
  ...
  if (store.feeds.length === 0) makeFeeds(api.getData())
...
`
  ...
  for (
    ...
    newsList.push(`
    <div class="p-6 ${
      store.feeds[i].read ? "bg-red-500" : "bg-white"
    } mt-6 rounded-lg shadow-md transition-colors duration-500 hover:bg-green-100">
    <div class="flex">
      <div class="flex-auto">
        <a href="#/show/${store.feeds[i].id}">${store.feeds[i].title}</a>  
      </div>
      <div class="text-center text-sm">
        <div class="w-10 text-white bg-green-300 rounded-lg px-0 py-2">${
          store.feeds[i].comments_count
        }</div>
      </div>
    </div>
    <div class="flex mt-3">
      <div class="grid grid-cols-3 text-sm text-gray-500">
        <div><i class="fas fa-user mr-1"></i>${store.feeds[i].user}</div>
        <div><i class="fas fa-heart mr-1"></i>${store.feeds[i].points}</div>
        <div><i class="far fa-clock mr-1"></i>${store.feeds[i].time_ago}</div>
      </div>  
    </div>
  </div>
  `)
